...
fs.writeFileSync('./artifactData/file0-1.txt', "", 'utf8');
fs.writeFileSync('./artifactData/file0-2.txt', "", 'utf8');
...
async function init() {
  ...
  ganglion.stream.pipe(
    ...
    if(flag == 0) {
      ...
      fs.appendFile('./artifactData/file0-1.txt', data.data[1].toString(), 'utf8', function (error) {
        if (error) {
          console.log(error)
        }
      })
      fs.appendFile('./artifactData/file0-2.txt', data.data[2].toString(), 'utf8', function (error) {
        if (error) {
          console.log(error)
        }
      })
      ...
      if(count <= json.count) {
        fs.appendFile('./artifactData/file0-1.txt', ',', 'utf8', function (error) {
          if (error) {
            console.log(error)
          }
        })
        fs.appendFile('./artifactData/file0-2.txt', ',', 'utf8', function (error) {
          if (error) {
            console.log(error)
          }
        })
